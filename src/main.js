...
function drawSimpleScoreHud() {
  const { chances } = missTracker.getState();
  const filledChances = String.fromCodePoint(9679).repeat(chances);
  const emptyChances = String.fromCodePoint(9675).repeat(GAME_CONFIG.initialChances - chances);
  const margin = GAME_CONFIG.layoutMode === 'PORTRAIT_MOBILE' ? 12 : 16;
  const centerX = GAME_CONFIG.baseWidth / 2;

  context.save();
  context.fillStyle = '#2F2418';
  context.font = `800 ${GAME_CONFIG.layoutMode === 'PORTRAIT_MOBILE' ? 16 : 18}px system-ui, sans-serif`;
  context.textBaseline = 'middle';
  context.textAlign = 'left';
  context.fillText(`SCORE ${scoreManager.getDisplayScore()}`, margin, 30);
  context.textAlign = 'center';
  context.fillText(`STAGE ${stageManager.getStage()}`, centerX, 30);
  context.font = `800 ${GAME_CONFIG.layoutMode === 'PORTRAIT_MOBILE' ? 12 : 14}px system-ui, sans-serif`;
  context.textAlign = 'right';
  context.fillText(`CHANCES ${filledChances}${emptyChances}`, GAME_CONFIG.baseWidth - margin, 30);
  context.restore();
}
